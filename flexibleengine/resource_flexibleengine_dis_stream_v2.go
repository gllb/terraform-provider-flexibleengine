	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"partition_count": {
				Type:     schema.TypeInt,
				Required: true,
				// ForceNew: true, TODO: Check if update do the job
				// ValidateFunc: validPartitionCount,
			},
			"data_duration": {
				Type:     schema.TypeInt,
				Optional: true,
...
func resourceDisStreamRead(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] Query dis stream using stream_name: %s", streamName)
	...
	if streamDetail != nil {
		d.Set("stream_name", streamDetail.StreamName)
		// d.Set("stream_type", streamDetail.StreamType)
...
// func validatePartitionCount(val interface{}, key string) (warns []string, err []error) {
// }
